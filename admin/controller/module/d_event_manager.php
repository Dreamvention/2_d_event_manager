...
class ControllerModuleDEventManager extends Controller {
	...
	public function index(){
		...
		if (isset($this->request->get['filter_event_action'])) {
			$url_params['filter_event_action'] = urlencode(html_entity_decode($this->request->get['filter_event_action'], ENT_QUOTES, 'UTF-8'));
		}
		...
		$data['column_event_action'] = $this->language->get('column_event_action');
		...
		$data['entry_event_action'] = $this->language->get('entry_event_action');
		...
		$filter_event_action = (isset($this->request->get['filter_event_action'])) ? $this->request->get['filter_event_action'] : null;
		...
		$filter_data = array(
			...
			'filter_event_action'      => $filter_event_action,
		...
		foreach ($results as $result) {
			...
			$enable = $this->url->link($this->route.'/enable', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'] . $url, 'SSL');
			$disable = $this->url->link($this->route.'/disable', 'token=' . $this->session->data['token'] . '&event_id=' . $result['event_id'] . $url, 'SSL');
		...
		$data['sort_event_action'] = $this->url->link($this->route, 'token=' . $this->session->data['token'] . '&sort=event_action' . $url, 'SSL');
		...
		$data['filter_event_action'] = $filter_event_action;
	...
	**/	
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_code']) || isset($this->request->get['filter_trigger']) || isset($this->request->get['filter_event_action'])) {
			if (isset($this->request->get['filter_code'])) {
			...
			if (isset($this->request->get['filter_event_action'])) {
				$filter_event_action = $this->request->get['filter_event_action'];
			} else {
				$filter_event_action = '';
			}
			...
			foreach ($results as $result) {
				$json[] = array(
					...
					'event_action'      => strip_tags(html_entity_decode($result['action'], ENT_QUOTES, 'UTF-8')),
				);
			}
		}
